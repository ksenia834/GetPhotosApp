...
final class PhotoGalleryViewController: UIViewController, StoryboardIdentifiable {
    ...
    override func viewDidLoad() {
        ...
        manager.photoLibraryDidChanged = { [weak self] change in
            guard let fetchResult = self?.assets, change.changeDetails(for: fetchResult) != nil,
                let collection = self?.currentCollection else { return }
            
            DispatchQueue.main.async {
                self?.loadAssets(from: collection)
            }
        }
    }
    ...
    private func loadCollections() {
        ...
        auth.requestPhotoLibraryAuthorization { [weak self] authorezed in
            ...
            strongSelf.collections = strongSelf.manager.fetchAssetCollections(excluding: [.smartAlbumVideos, .smartAlbumTimelapses,
                                                                                          ...
                                                                                          .smartAlbumPanoramas, .smartAlbumAnimated,
                                                                                          .smartAlbumBursts, .smartAlbumAllHidden])
    ...
    private func showPhotoLibraryCollections() {
        if collectionsViewController == nil {
            ...
            guard let viewController = storyboard.instantiateViewController(withIdentifier: "CollectionsViewController")
                as? CollectionsViewController else { return }
...
extension PhotoGalleryViewController: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let asset = assets?[indexPath.row] {
            ...
            let cellBounds = cell.imageView.bounds

            manager.loadThumbnailFromAsset(asset: asset,
                                           targetSize: CGSize(width: cellBounds.width * scale, height: cellBounds.height * scale),
                                           completion: { image in
